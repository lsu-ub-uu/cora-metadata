...
exportRecords(){
	...
	local sql="select * from record where datadivider = '"$dataDivider"'"
...
exportLinks(){
	...
	local sql="select l.* from link l join record r on l.fromtype = r.\"type\"  and l.fromid = r.id where datadivider = '"$dataDivider"'"
